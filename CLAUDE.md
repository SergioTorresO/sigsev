...
| GET | /api/catalog/categories, /api/catalog/signal-types | ✅ cualquier rol | Listar categorías de señal (con conteo de tipos) y tipos de señal (paginado, filtra por `category_id`/`search`) — usados por la página de administración `/dashboard/admin/catalogo` |
| POST/PUT/DELETE | /api/catalog/categories(/:id), /api/catalog/signal-types(/:id) | ✅ ADMIN | CRUD de categorías y tipos de señal; eliminar falla con mensaje claro si hay tipos/señales asociadas; código de tipo de señal único (se normaliza a mayúsculas) |
...
| Gestionar catálogo de categorías/tipos de señal (`/dashboard/admin/catalogo`) | ✅ | ❌ | ❌ | ❌ |
...
21. **`<Modal>`/`<Pagination>` reutilizables (frontend)**: los patrones de overlay `fixed inset-0` + panel de diálogo, y de "Página X de Y" + Anterior/Siguiente, estaban duplicados casi al pie de la letra en cada página con modales/paginación. `components/Modal.tsx` centraliza overlay, `role="dialog"`/`aria-modal`/`aria-labelledby`, botón de cierre opcional (`showCloseButton`), ancho configurable (`maxWidthClassName`) y llama internamente a `useModalA11y` (focus trap, cierre con Escape, autofocus, restauración de foco) para que cada página no tenga que volver a aplicar esa lógica. `components/Pagination.tsx` centraliza el control de paginación y no renderiza nada si `totalPages <= 1`. Adoptados en las 8 páginas con modales/paginación: `signals`, `zonas`, `inspections`, `maintenances`, `mis-asignaciones`, `admin/users`, `admin/audit`, `admin/catalogo`. La componentización de una `<Table>` genérica se evaluó y se descartó deliberadamente: los esquemas de columnas difieren demasiado entre páginas (señales, zonas, usuarios, auditoría) para que la abstracción resultante fuera más simple que el HTML que reemplazaría.
22. **`catalog` (CRUD) vs `references` (solo lectura) para categorías/tipos de señal**: son dos módulos de backend distintos sobre las mismas tablas (`signal_categories`, `signal_types`) a propósito. `references/` (`GET /api/ref/categories`, `/api/ref/signal-types`) es el endpoint liviano, abierto a cualquier rol, que consumen los formularios/filtros de señales (selects simples, sin paginar). `catalog/` (`GET/POST/PUT/DELETE /api/catalog/categories`, `/api/catalog/signal-types`) es el CRUD completo — escritura exclusiva de ADMIN, con paginación/búsqueda en tipos de señal, conteo de tipos por categoría, código único normalizado a mayúsculas, y guardas de integridad (no se borra una categoría con tipos asociados ni un tipo usado por señales) — que alimenta la página de administración `/dashboard/admin/catalogo`. No se fusionaron para no acoplar la lectura pública liviana con la lógica de administración.
23. **`ToastContext` memoiza su `value` con `useMemo`**: antes, `success`/`error`/`info` se recreaban como funciones nuevas en cada render de `ToastProvider`, así que `value` cambiaba de identidad en cada render aunque el contenido fuera igual. Cualquier página que use `toast` como dependencia de un `useCallback`/`useEffect` (patrón usado en casi todas las páginas con fetch) entraba en loop infinito (`Maximum update depth exceeded`) — el primer caso detectado fue `/dashboard/admin/catalogo`, pero el bug era transversal a toda la app. `value` ahora depende solo de `showToast` (ya estable vía `useCallback`).
24. **`apiFetch` (`frontend/src/lib/api.ts`) parsea el body de forma defensiva**: antes hacía `res.json()` a ciegas; si el backend respondía texto plano (p.ej. el mensaje por defecto de `express-rate-limit` en un 429), el `JSON.parse` implícito reventaba con `Unexpected token`. Ahora lee `res.text()` y hace `JSON.parse` en un try/catch, cayendo a `{ message: rawText }` si no es JSON válido — nunca crashea por un body inesperado.
25. **`express-rate-limit` responde JSON, no texto plano**: el `message` del limiter general en `server.ts` es un objeto (`{ message: '...' }`) en vez de un string — `res.send()` de Express serializa objetos como JSON automáticamente, así que el 429 es consistente con el resto de la API (y con lo que `apiFetch` espera).
26. **`GET /api/ref/municipalities` sin `department_id` pagina internamente**: PostgREST limita cada respuesta a `max_rows` (1000 por defecto). El catálogo tiene ~1120 municipios, así que sin paginar la mitad final alfabética (desde "Tenza" en adelante — Tuluá, Turbo, Valledupar, Villavicencio, Zipaquirá, etc.) se perdía en silencio para cualquier consumidor del catálogo completo, en particular el cascada Departamento→Municipio de `/dashboard/zonas` (que carga todo de una vez para filtrar en cliente). La ruta ahora pagina con `.range()` en bloques de 1000 hasta traer todo. Nota pendiente: **los nombres de varios municipios están guardados sin tildes** en los datos (ej. `"Medellin"`, `"Tulua"`, `"Zipaquira"`) — no es un bug de código, es un problema del dataset cargado directo en Supabase (no hay script/CSV fuente versionado en el repo, ver "Próximos pasos").
...
- [x] Módulo de administración de catálogo (`/dashboard/admin/catalogo`, solo ADMIN): CRUD de categorías de señal y tipos de señal, con paginación/búsqueda y guardas de integridad (ver decisión 22)
...
- [ ] **Corregir tildes faltantes en `municipalities.name`**: varios nombres están guardados sin diacríticos (`"Medellin"`, `"Tulua"`, `"Zipaquira"`, etc.). No hay script/CSV fuente versionado en el repo para regenerar los datos (se cargaron directo en Supabase, fuera del código — ver decisión 26), así que hace falta un dataset DANE con tildes correctas para hacer una corrección puntual vía SQL; no se debe intentar "adivinar" acentos por código sobre los nombres existentes.
